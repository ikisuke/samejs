...
class RandomAudioProcessor extends AudioWorkletProcessor {
  constructor(options) {
    ...
    this._audio_reader = new AudioReader(
      new RingBuffer(denoisedSab, Float32Array)
    );
  }
  ...
  process(inputList, outputList, parameters) {
    ...
    if (this._audio_writer.enqueue(inputList[0][0]) !== 128) {
      console.log(
        "audioworklet underrun: the worker doesn't dequeue fast enough!"
      );
    }
    ...
    if (this._audio_reader.available_read() >= 128) {
      ...
      if (samples_read < 128) {
        console.log(
          `ERROR! this._audio_reader in audioworklet read ${samples_read}`
        );
      ...
      console.log(
        `audioworklet underrun: the worker doesn't enqueue fast enough! available only - ${this._audio_reader.available_read()}`
      );
    }
